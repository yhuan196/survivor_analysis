...
surv_model_cox1 <- coxph(data = survivor_data_final,
  Surv(time, status) ~ gender + poc + age_during_show + personality_type_binary + region)
# summary(surv_model_cox1)
#Clean table for Cox PH
tabcoxph(surv_model_cox1, 
         var.labels = list(gender = "Gender", poc = "POC", age_during_show = "Age", personality_type_binary = "Personality Type",  region = "Region"), 
         decimals = 4)
```
...
We use `ggsurvplot` function to draw the survival curves and calculate log-rank test p-value.
